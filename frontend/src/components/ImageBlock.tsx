    ...
    return (
      <div className="group/img relative w-full">
        <img
          src={data.src}
          alt=""
          className="w-full rounded-xl border border-[#1B1326]/[0.08] object-contain"
        />
        <button
          onClick={() => setSrc('')}
          title="Remove image"
          className="absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-lg bg-white/90 text-[#6E5F7B] opacity-0 shadow-sm backdrop-blur transition-all hover:text-[#B91C1C] group-hover/img:opacity-100 print:hidden"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
  ...
  return (
    <div
      ...
      className={`flex w-full cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed px-6 py-10 text-center transition-all print:hidden ${
        isDragOver ? 'border-[var(--accent)] bg-[var(--accent-tint)]' : 'border-[#1B1326]/15 hover:border-[#1B1326]/30 hover:bg-[#1B1326]/[0.02]'
      }`}
    >
      ...
      <div className="flex flex-col gap-0.5">
        <span className="text-[14px] font-bold text-[#1B1326]">
          {isDragOver ? 'Drop to add image' : 'Drag & drop an image'}
        </span>
        <span className="text-[12px] text-[#6E5F7B]">or click to browse files</span>
      </div>
      <input
        ref={inputRef}
        type="file"
