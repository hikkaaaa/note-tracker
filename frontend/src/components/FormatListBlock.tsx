...
export function FormatListBlock({ content, onChange }: { content: string, onChange: (c: string) => void }) {
  ...
  return (
    <div className="w-full relative group/list block">
      {/* Dynamic List Style Dropdown */}
      <div className="absolute -left-11 -top-8 opacity-0 group-hover/list:opacity-100 transition-opacity print:hidden">
        <button 
          onClick={() => setShowMenu(!showMenu)} 
          className="p-1 rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-100 flex items-center bg-white shadow-sm border border-slate-200"
        ...
        {showMenu && (
          <div className="absolute top-full -left-2 mt-1 bg-white border border-slate-200 shadow-lg rounded-lg py-1 z-30 w-32" onMouseLeave={() => setShowMenu(false)}>
            {styles.map(s => (
              <button 
                key={s} 
      ...
      <div className="space-y-1">
        {items.map((item, index) => (
          <div key={item.id} className="flex items-start gap-2 group/item">
            <span className="mt-1 min-w-[1.5rem] font-medium text-slate-500 text-sm text-right shrink-0">
            ...
            <button 
              ...
              className="opacity-0 group-hover/item:opacity-100 p-1 text-slate-300 hover:text-red-400 transition-opacity print:hidden"
            >
      ...
      <button 
        onClick={() => handleAdd()}
        className="mt-3 text-xs font-semibold text-slate-400 hover:text-slate-600 flex items-center gap-1 transition-colors pl-8 print:hidden"
      >
