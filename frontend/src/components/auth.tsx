...
function AppleG() {
  return (
    <svg width="16" height="18" viewBox="0 0 24 24" fill="#1B1326">
...
function GitHubG() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="#1B1326">
  ...
  return (
    <div className="relative rounded-[24px] border border-[rgba(27,19,38,0.08)] bg-white p-9 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_30px_60px_-28px_rgba(119,88,163,0.35),0_8px_20px_-10px_rgba(27,19,38,0.12)]">
      <div
        ...
        style={{
          background:
            'linear-gradient(135deg, rgba(119,88,163,0.25), transparent 40%, rgba(219,62,140,0.18))',
        }}
      />
      {children}
    </div>
...
export function FormHead({ title, subtitle, titleSize = 32 }: { title: string; subtitle: string; titleSize?: number }) {
  return (
    <div className="mb-6 text-center">
      ...
      <p className="m-0 text-sm text-[#6E5F7B]">{subtitle}</p>
    </div>
  )
}
    ...
    <div className="mb-5 grid grid-cols-3 gap-2">
      {socials.map((s) => (
        <button
          key={s.label}
          type="button"
          className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-[rgba(27,19,38,0.08)] bg-[#FBF7F2] px-2 py-3 text-[13px] font-semibold text-[#1B1326] transition-all hover:-translate-y-px hover:bg-white"
        >
...
export function AuthDivider() {
  return (
    <div
      className="mb-[18px] mt-1 flex items-center gap-3 text-[12px] uppercase tracking-[0.1em] text-[#6E5F7B]"
      style={{ fontFamily: "'Geist Mono', monospace" }}
    >
      <span className="h-px flex-1 bg-[rgba(27,19,38,0.08)]" />
      or with email
      <span className="h-px flex-1 bg-[rgba(27,19,38,0.08)]" />
    </div>
  )
  ...
  return (
    <span
      className={`grid h-[18px] w-[18px] flex-shrink-0 place-items-center rounded-[5px] border-[1.5px] transition-all ${
        checked ? 'border-[#7758A3] bg-[#7758A3]' : 'border-[#CFC5D8] bg-white'
      }`}
    >
      <svg
        width="12"
...
export function AuthSuccess({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="flex flex-col items-center gap-3.5 py-[30px] text-center">
      <motion.div
        className="grid h-[72px] w-[72px] place-items-center rounded-full bg-[#7758A3] shadow-[0_0_0_8px_rgba(119,88,163,0.15)]"
      ...
      <p className="m-0 text-sm text-[#6E5F7B]">{subtitle}</p>
      <div className="mt-3 h-1 w-[200px] overflow-hidden rounded-full bg-[#7758A3]/10">
        <span
          className="block h-full w-1/2 rounded-full"
          style={{
            background: 'linear-gradient(90deg, #7758A3, #DB3E8C)',
